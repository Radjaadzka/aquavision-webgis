...
class SumberAir(models.Model):
    ...
    # Field access config: which fields each role can see
    USER_FIELDS = ['nama', 'jenis_sumber', 'kondisi', 'lat', 'lon']
    ADMIN_FIELDS = ['nama', 'jenis_sumber', 'kondisi', 'debit', 'tahun_pendataan', 'lat', 'lon', 'easting', 'northing']
...
class FasilitasWisata(models.Model):
    ...
    ADMIN_FIELDS = ['nama', 'jenis', 'kamar', 'kapasitas', 'kebutuhan_air_harian', 'lat', 'lon', 'easting', 'northing']
...
class Permukiman(models.Model):
    ...
    kategori_pelanggan = models.CharField(max_length=50, choices=KATEGORI_CHOICES, blank=True, null=True)
    ...
    ADMIN_FIELDS = ['nama_dusun', 'jumlah_kk', 'jumlah_penduduk', 'kategori_pelanggan', 'rata_rata_kebutuhan', 'lat', 'lon', 'easting', 'northing']
...
class Feedback(models.Model):
    ...
    def __str__(self):
        return f"{self.nama} - {self.tanggal.strftime('%d/%m/%Y')}"